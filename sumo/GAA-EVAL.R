...
groupFilter = function(real.data, pred.data,top = NA) {
  ...
  if (!is.na(top)) {
    max.r = max.r %>% arrange(desc(max.r))
    max.r = max.r[1:top,]
  }
...
eval.AUC <- function(real.data, pred.data, threshold = 0.5, sd.use = NA, lower.positive=F,z.transform = F){
  sd.enable <- is.numeric(sd.use) & length(real.data$sd) >1
  if(sd.enable){
    keep <- (real.data$sd < sd.use) & (!is.na(real.data$sd))
    real.data$value <- real.data$value[keep]
  ...
  ret = list(sd.use = sd.use,threshold = threshold, realClass = realClass, results = round(auc.summary,4), pred.data = pred.data, real.data = real.data)
  return(ret)
}
...
plot.AUC.helper = function(file.name,results) {
  ...
  sd.enable <- is.numeric(results$sd.use) & length(results$real.data$sd) >1
  if (sd.enable) {
    title = paste0(file.name, "thresh=", results$threshold, "sd<", results$sd.use, "ROCcurve")
  } else {
    title = paste0(file.name, "thresh=", results$threshold, results$sd.use, "ROCcurve")
  }
  # get the evaluated statistics
  rocobj <- roc(results$realClass,results$pred.data$value[,file.name])
...
plot.AUC = function(ret) {
  ...
  plots <- lapply(colnames(ret$pred.data$value), 
                        function(x) plot.AUC.helper(x,ret) 
                        )
...
eval.uniqueness = function(real.data,pred.data,boot = FALSE, top = NA) {
  ######### debug input  #####################
  # real.data <- read.RealData(file = "exp_data.csv", sep = ",",
  #                            col.id = 2, col.value = 5, col.sd = 6)
  # pred.data <- read.Submission.Folder(folder.name = "prediction/",col.id = 1,
