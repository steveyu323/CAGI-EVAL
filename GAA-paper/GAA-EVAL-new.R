...
eval.AUC.only <-
  function(real.data,
    ...
    if ((sum(realClass) < 5) ||
        (sum(realClass) > length(realClass) - 5)) {
      return (eval.AUC.only(real.data,
                            pred.data,
                            boot = FALSE))
    }
...
plot.AUC.helper = function(file.name, results,return.ob = F) {
...
plot.AUC.combined = function(ret) {
  obs <- lapply(colnames(ret$pred.data$value),
                  function(x)
                    plot.AUC.helper(x, ret,return.ob = T))
  names(obs) = colnames(ret$pred.data$value)
  g = ggroc(obs)
  return(g)
}
  ...
  function(result.cor,
    ...
    mydata <-
      data.frame(submissions = row.names(result.cor),
                 Value = result.cor[, 1])
    plot <-
      ggplot(data = mydata, aes(x = submissions, y = Value)) +
    ...
    plot = plot + ggtitle(paste("Plot of", method, " vs. Predictor", sep =
