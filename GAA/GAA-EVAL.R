...
Validate.Submission.File <- function(infile.path, template.file,
  ...
  if (number.of.rows != nrow(pred.table)) { 
    cat('submission file ', infile.path,
  ...
  if (any(grepl('NA', pred.table) == TRUE)){
    cat('submission file ', infile.path, ' has a blank cell in column ',
  ...
  valid.column.1 = TRUE
  if (any(sort(as.character(correct.column.1)) !=  sort(as.character(pred.table[, 1])))) {       # escape sequences \d, \s and \w represent any decimal digit, space character and ‘word??? character, * means zero or one match,  + means preceding item will match one or more times
    cat("First column of ", infile.path," must contain valid alleles! Check row ",
  ...
  if (any(regexpr('[*]|\\d+', pred.table[, 2]) != 1)) {
    cat("Second column of ", infile.path," must be numeric or \"*\"!", '\n')
    ...
    cat("Second column of ", infile.path, " must be numeric or \"*\", not both!",'\n')
    valid.column.2 = FALSE
  }
  #check if column 3 is numerical value or star
  valid.column.3 = TRUE
  if (any(regexpr('[*]|\\d+', pred.table[, 3]) != 1)) {
    cat("Third column of ", infile.path, " must be numeric or \"*\"!",'\n')
    ...
    cat("Third column of ", infile.path, " must be numeric or \"*\", not both!",'\n')
    valid.column.3 = FALSE
  }
...
eval.RMSD <- function(real.data, pred.data, sd.use = NA, 
  ...
  if (boot) {
      ...
      real.rep = rep.obj$real.rep
      pred.rep = rep.obj$pred.rep
      rmsd.rep = lapply(1:rep.time,function(x) eval.RMSD(real.data = real.rep[[x]], pred.data = pred.rep[[x]], sd.use, 
                                                        density.distance, density.distance.adjust, variance.normalization,
                                                        boot = FALSE))
      rmsd.rep = list.cbind(lapply(1:rep.time,function(x) rmsd.rep[[x]][,1]))
      row.names(rmsd.rep) = row.names(rmsd.rep[[1]])
      
      ci_low = list.rbind(lapply(1:nrow(rmsd.rep),function(x) quantile(rmsd.rep[x,],probs=0.05)))
      ci_high = list.rbind(lapply(1:nrow(rmsd.rep),function(x) quantile(rmsd.rep[x,],probs=0.95)))
      sd = list.rbind(lapply(1:nrow(rmsd.rep),function(x) sd(rmsd.rep[x,])))
      avg = list.rbind(lapply(1:nrow(rmsd.rep),function(x) mean(rmsd.rep[x,])))
...
eval.AUC <- function(real.data, pred.data, threshold = 0.5, sd.use = NA, lower.positive=F){
  ...
  AUC.Helper <- function(file.name,lower.positive){
    ...
    predClass <- AUC.build.class(pred.data$value[,file.name],lower.positive,threshold)
     #get positive
    ...
    pred <<- prediction( predictions= pred.data$value[,file.name], labels= realClass)
    perf <<- performance(pred, "tpr", "fpr")
    auc <- performance(pred, "auc")
    auc <- round(unlist(slot(auc, "y.values")), digits = 6)
    result <- c(AUC = auc, sensitivity = sens, specificity = spec,accuracy = accuracy,precision = precision,f1_score = f1_score,bAccu = bAccu)
    return(result)
  }
  ...
  if(sd.enable){
    colnames(auc.summary)[1] <- paste(colnames(auc.summary)[1], ".n=",n,".sd<", sd.use, sep="")
  }
  return(round(auc.summary,4))
}
...
plot.AUC.helper = function(real.data,pred.data, file.name,threshold, sd.use, lower.positive,results) {
  # render binary class data
  realClass <- AUC.build.class(real.data$value,lower.positive,threshold)
  # sd.enable dependent plot title
  sd.enable <- is.numeric(sd.use) & length(real.data$sd) >1
  if (sd.enable) {
    title = paste0(file.name, "thresh=", threshold, "sd<", sd.use, "ROCcurve")
  } else {
    title = paste0(file.name, "thresh=", threshold, sd.use, "ROCcurve")
  }
  # get the evaluated statistics
  rocobj <- roc(realClass,pred.data$value[,file.name])
  result.label = paste(colnames(results) , results[file.name,],sep = ":")
  g <- ggroc(rocobj,legacy.axes = TRUE)
  g = g + theme_minimal(base_size = 5) + ggtitle(title) + 
    geom_segment(aes(x = 0, xend = 1, y = 0, yend = 1), color="grey", linetype="dashed") +
    annotate("text", x = 0.75, y = seq(0.125, 0.5, 3/64)[1:7], label = result.label,size = 2)
  return(g)
}
...
#' @param lower.positive whether lower than threshold is used as positive binary value
#' @return a ggplot object with all the ROC plots arranged
plot.AUC = function(real.data, pred.data, threshold = 0.5, sd.use = NA, lower.positive=F) {
  ##########################################
  # # for debugging input
  # real.data <- read.RealData(file = "exp_data.csv", sep = ",",
  #                            col.id = 2, col.value = 5, col.sd = 6)
  ...
  results = eval.AUC(real.data, pred.data, threshold, sd.use, lower.positive)
  plots <- lapply(colnames(pred.data$value), 
                        function(x) plot.AUC.helper(real.data,pred.data,x,threshold,sd.use,lower.positive,results) 
                        )
...
eval.Correlation.Between <- function(real.data, pred.data, method = "pearson", 
                                     sd.use = NA,z.transform = TRUE, grouped = FALSE,  ...){
  ###############################################
  # for debugging input
  # real.data <- read.RealData(file = "exp_data.csv", sep = ",",
  #                            col.id = 2, col.value = 5, col.sd = 6)
...
eval.uniqueness = function(real.data,pred.data,boot = FALSE) {
  ######### debug input  #####################
  # real.data <- read.RealData(file = "exp_data.csv", sep = ",",
  #                            col.id = 2, col.value = 5, col.sd = 6)
  # pred.data <- read.Submission.Folder(folder.name = "prediction/",col.id = 1,
  ...
  grouped.pred.data = groupFilter(real.data,pred.data)
  input.group = lapply(1:length(colnames(grouped.pred.data$value)),function(x) colnames(grouped.pred.data$value)[-x] )
  r.square.excl = unlist(lapply(1:length(input.group),function(x) lmFitHelpher(real.data,grouped.pred.data,input.group[[x]])))
  r.square.tot = lmFitHelpher(real.data,grouped.pred.data,colnames(grouped.pred.data$value))
  uniqueness = r.square.tot - r.square.excl
  result = data.frame(uniqueness)
  row.names(result) = grouped.pred.data$group$group
  ...
  if (boot) {
    ...
    real.rep = rep.obj$real.rep
    pred.rep = rep.obj$pred.rep
    unique.rep = lapply(1:rep.time,function(x) eval.uniqueness(real.data = real.rep[[x]], pred.data = pred.rep[[x]]))
    rname = row.names(unique.rep[[1]])
    unique.rep = list.cbind(lapply(1:rep.time,function(x) unique.rep[[x]][,1]))
    ci_low = list.rbind(lapply(1:nrow(unique.rep),function(x) quantile(unique.rep[x,],probs=0.05)))
    ci_high = list.rbind(lapply(1:nrow(unique.rep),function(x) quantile(unique.rep[x,],probs=0.95)))
    sd = list.rbind(lapply(1:nrow(unique.rep),function(x) sd(unique.rep[x,])))
    avg = list.rbind(lapply(1:nrow(unique.rep),function(x) mean(unique.rep[x,])))
...
plot.Correlation <- function(result.cor, method, pval=TRUE,boot = FALSE){
  mydata <-data.frame(submissions=row.names(result.cor), Correlation=result.cor[,1])
  plot <- ggplot(data=mydata, aes(x=submissions, y=Correlation)) + 
  ...
  if (boot) {
    plot2 <- plot2 + geom_errorbar(aes(ymin=result.cor$low_ci, ymax=result.cor$high_ci),
...
plot.Correlation.Between <- function(result.cor, method, color_low="blue", color_high="red", color_mid="white") {
  ...
  p <- ggplot(data = result.cor.melted, aes(result.cor.melted$Var1, result.cor.melted$Var2, fill = value)) +   
...
plot_single_scatter = function(real.data,pred.data,file.name) {
  ...
  p = ggplot(dat, aes(x=real.value, y=pred.value)) +
    ...
    labs(title= plot.title,
         x="Experimental Value", y = file.name, cex.lab=1.5)+
    theme_classic() 
  # Plot
  p = p+
    geom_segment(aes(x = min(real.value),y = min(real.value), xend = max(real.value),yend = max(real.value)))
  p
  return(p)
}
...
plot_all_scatter = function(real.data,pred.data, z.transform = TRUE) {
  ...
  plots <- lapply(colnames(pred.data$value),function(x) plot_single_scatter(real.data,pred.data,x))
  ml <- marrangeGrob(plots, nrow=2, ncol=2)
  return(ml)
}
...
plot.RMSD <- function(result.rmsd, method="",boot = FALSE,...) {
  mydata <-data.frame(submissions=row.names(result.rmsd), rmsd=result.rmsd[,1])
  head(mydata)
  ...
  if (boot) {
    p <- p + geom_errorbar(aes(ymin=result.rmsd$low_ci, ymax=result.rmsd$high_ci),
                                   width=.2,position=position_dodge(.9))
  }
  return(p)
...
plot.uniqueness <- function(result.uniq, method="",boot = FALSE) {
  mydata <-data.frame(submissions.group=row.names(result.uniq), uniqueness=result.uniq[,1])
  p <- ggplot(data=mydata, aes(x=submissions.group, y=uniqueness)) + 
    geom_bar(stat="identity") + theme_classic() + 
    theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) + 
    ggtitle(paste("Plot of","Unique Linear Model contribution of each group", sep=" ")) + theme(plot.title = element_text(hjust = 0.5))
  if (boot) {
    p = p + geom_errorbar(aes(ymin=result.uniq$low_ci, ymax=result.uniq$high_ci),
                              width=.2,position=position_dodge(.9))
  }
  return(p)
}
